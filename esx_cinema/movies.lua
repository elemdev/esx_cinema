...
-- Check if the cinema is open or closed.
          if hour < openingHour or hour > closingHour then
			helpDisplay("O cinema está ~r~fechado ~w~, por favor volta entre a 01:00 e 22:00.", 0)
          else
			helpDisplay("Pressiona ~INPUT_CONTEXT~ para entrares na sala de cinema.", 0)
				...
				function(data, menu)
					...
					ESX.TriggerServerCallback('esx_cinema:checkmoney',function(valid)
						if (valid) then
							...
							AddTextComponentString("Pressiona ~r~E ~w~para sair da sala de cinema.")
							DrawNotification(false, false)
						else
							TriggerEvent('esx:showNotification', "Não tens dinheiro suficiente!")
						end
					end)
